...
def text_process(text: str, task_type: str, prod_type: str) -> dict:
    """
    根据任务类型处理文本并返回相应格式的响应
    """
    if task_type == "产品市场概览":
        df = get_insurance_data_with_cache(prod_type).tail(5)
        print(df)
        dataframe_data = {
            "columns": df.columns.tolist(),
            "data": df.values.tolist()
    ...
    #         "dataframe": dataframe_data
    #     }
    elif task_type == "保险宏观分析":
        # prompt = prompt_for_task_distribution(text)
        # detailed_response = call_deepseek_local(prompt)
        try:
            detailed_response = RAG_main(text)
...
def process_text():
    ...
    try:
        ...
        # print(prompt)
        task_result = parse_llm_output(call_deepseek_local(prompt), Query)
        task_type = task_result.get('task_type', '未知任务')

        print("====================正在尝试调用服务器工具========================")
        # 根据任务类型处理文本
        response_data = text_process(text, task_type,task_result.get('prod_type', '健康险'))
